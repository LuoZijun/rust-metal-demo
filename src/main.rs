...
fn render() {
    ...
    let glutin_window = winit::WindowBuilder::new()
        ...
        .with_title("Preview With Metal".to_string())
    ...
    let library = device.new_library_with_data(&include_bytes!("./default.metallib")[..]).unwrap();
    // let library = device.new_library_with_source(include_str!("./shaders.metal"), &metal::CompileOptions::new()).unwrap();
...
fn main() {
    println!("Devices:");
    for device in metal::Device::all().iter() {
